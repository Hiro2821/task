(() => {
  ...
  const weekTotalLabel = document.getElementById("weekTotalLabel");
  const weekTotalFill = document.getElementById("weekTotalFill");
  const weekDayList = document.getElementById("weekDayList");
  const closeWeeklyBtn = document.getElementById("closeWeekly");
  ...
  function openWeekly() {
    weekViewDate = new Date(currentDate);
    renderWeekly();
    weeklyOverlay.classList.remove("hidden");
  }

  function closeWeekly() {
    weeklyOverlay.classList.add("hidden");
  }

  function renderWeekly() {
    subjects = loadSubjects();
    const monday = mondayOf(weekViewDate);
    const sunday = addDays(monday, 6);
    weekRangeLabel.textContent = `${formatMD(monday)} 〜 ${formatMD(sunday)}`;
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const key = dateKey(d);
      const dKey = dayKeyForDate(d);
      const tasks = getTasksForDate(key, dKey);
      const total = tasks.length;
      const done = tasks.filter((t) => t.done).length;
      totalAll += total;
      doneAll += done;
      ...
      const row = document.createElement("div");
      row.className = "week-day-row" + (key === todayKey ? " is-today" : "");

      const label = document.createElement("div");
      label.className = "week-day-label";
      label.textContent = `${DAY_LABELS[dKey]} ${d.getMonth() + 1}/${d.getDate()}`;

      const track = document.createElement("div");
      track.className = "progress-bar-track small";
      ...
      count.textContent = total ? `${done} / ${total}` : "予定なし";
    ...
    weekTotalLabel.textContent = `${doneAll} / ${totalAll} 完了`;
    weekTotalFill.style.width = totalAll ? `${(doneAll / totalAll) * 100}%` : "0%";
  }

  weeklyBtn.addEventListener("click", openWeekly);
  closeWeeklyBtn.addEventListener("click", closeWeekly);
  weeklyOverlay.addEventListener("click", (e) => {
    if (e.target === weeklyOverlay) closeWeekly();
  });
  prevWeekBtn.addEventListener("click", () => {
    weekViewDate = addDays(weekViewDate, -7);
    renderWeekly();
  });
